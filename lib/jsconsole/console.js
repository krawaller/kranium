...
function puts(msg){
	var el = document.createElement('div'),
	  ...
	  span.innerHTML = prettyPrintOne(js_beautify(msg).replace(/& lt;/g, '&lt;').replace(/& gt;/g, '&gt;'));
...
//console.log(document.documentElement.scrollHeight - window.innerHeight - 50 , window.pageYOffset, document.documentElement.scrollHeight)
	  /*if(document.documentElement.scrollHeight - window.innerHeight - 50 > window.pageYOffset){
		console.log('scrolling to ', document.documentElement.scrollHeight);
	...
	// FIXME
	/*if(document.documentElement.scrollHeight - window.innerHeight - 50 > document.body.scrollTop){
		console.log('scrolling to ', document.documentElement.scrollHeight);
...
var socket;
function init(){
	socket = new io.Socket(null, {reconnect:true}); 
	socket.connect();
	socket.on('connect', function(){ puts('Console connected'); });
	socket.on('reconnect', function(){ puts('Console reconnected'); });
...
function run(cmd) {
  ...
  if (internalCmd) {
	...
	socket.send({ action: 'cmd', cmd: cmd });
    /*try {
...
function post(cmd, blind) {
  ...
  /*var el = document.createElement('div'),
      ...
      span = document.createElement('span'),
      parent = output.parentNode, 
      response = run(cmd, function(err, resp){
        console.log(resp);
      });
