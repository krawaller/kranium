if(K.is.android){
		...
		init: function(o){
			if(!this.navBarHidden && (this.leftNavButton || this.rightNavButton || K.settings.alwaysUseCustomAndroidNavBar)){
				...
				this.children = [
					...
					{
						type: 'tabbedbar',
						labels: ['hej', 'hopp', 'korv'],
						//items: [K.createView({ text: 'huh', backgroundColor: '#ff0' }), 'spacer', 'hopp', 'korv', K.create({ type: 'button', title: 'meh', width: 80 })],
						//items: [{ type: 'button', title: 'hej', width: 60 }, { type: 'tabbedbar', labels: ['1', '2', '3'] }],
						bottom: 0
					},
					{
						type: 'buttonbar',
						labels: ['buttonbar', 'hopp', 'korv'],
						//items: [K.createView({ text: 'huh', backgroundColor: '#ff0' }), 'spacer', 'hopp', 'korv', K.create({ type: 'button', title: 'meh', width: 80 })],
						//items: [{ type: 'button', title: 'hej', width: 60 }, { type: 'tabbedbar', labels: ['1', '2', '3'] }],
						bottom: 88
					},
	...
	K['createTabGroup'] = K.creators['tabgroup'] = function(opts){
		var tabBar = K.createView({
			height: 50,
			bottom: 0,
			children: [{
				type: 'tabbedbar',
				labels: opts.tabs.map(function(t){
					return t.title
				})
			}]
		});
	...
	K['createToolbar'] = K.creators['toolbar'] = function(opts){
		...
		K.log(' ====== numspac', numSpacers, spacerWidth);	
		var items = [];
		(opts.items||[]).forEach(function(o, i){
			K.log(' =============== ', 'left: ' + left, o, typeof o, K.stringify(o||{}));
			if(o === 'spacer'){
				left += spacerWidth;
			} else {
				if(typeof o === 'string'){
