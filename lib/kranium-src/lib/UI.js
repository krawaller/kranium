...
exports.UI = function(K, global){
	...
	.forEach(function(t){
		var type = t.toLowerCase(),
			...
			UI[t] = K.classes[type] = K.Class.extend({
		        type: type,
		        init: defaultInit
			});
		...
		if((extra = extraCreators[t])){
			...
			global[extra] = K.classes[extraType] = K.Class.extend({
		        type: type,
		        init: defaultInit
			});
	...
	function create(o, def, overriders){
		if(o instanceof Array){ return o.map(function(el){ return create(el, def); }); }
